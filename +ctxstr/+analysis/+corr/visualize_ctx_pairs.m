...
get_ylabel = @(i,j,c) sprintf('Str = %d\nStr = %d\n{\\it r} = %.4f',...
            ctx_traces.info.ind2rec(i), ctx_traces.info.ind2rec(j), c);
...
for p = 1:num_pages
    ...
    for r = 1:length(rows)
        ...
        traces1 = ctxstr.core.get_traces_for_cell(ctx1_idx, ctx_traces.trial);
        
        ctx2_idx = corrlist(row,2);
        traces2 = ctxstr.core.get_traces_for_cell(ctx2_idx, ctx_traces.trial);
