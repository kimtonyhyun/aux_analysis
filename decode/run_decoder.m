...
trials = ds.filter_trials('start', 'west'); % Changing path

[traces_perf, info] = decode_end(alg, ds, pos, trials, 'traces', num_runs);
...
subplot(122);
[raster, info] = ds.get_aligned_trace(cell_idx, trial_inds, info.pos_frames(trial_inds,1), 'apply_trial_offset');
imagesc(info.aligned_time, 1:num_trials, raster);
title(sprintf('Aligned to pos=%.1f', pos));

trial_inds = find(ds.filter_trials('start', 'west', 'end', 'north'));
[raster2, info2] = ds.get_aligned_trace(cell_idx, trial_inds, info.pos_frames(trial_inds,1), 'apply_trial_offset');
% title('West --> South');
% subplot(122);
