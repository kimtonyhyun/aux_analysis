function [A,C,newcenters,CC] = manually_refine_components2(Y,A,C,centers,img,sx,options,points_of_interest)
...
num_points = size(points_of_interest, 1);
for k = 1:num_points 
    x = points_of_interest(k,1);
    y = points_of_interest(k,2);
    hold on;
    if ~isempty(x)
        pixel=round([x y]);
        ...
        scatter(new_center(2),new_center(1),'r*'); hold on; 
